...
function parseArguments(arguments_: string[]): CliOptions | null {
  ...
  if (positional.length !== 1) {
    throw new TypeError("Exactly one verification JSON file is required");
  }

  const inputPath = positional[0];
  if (!inputPath) {
    throw new TypeError("Verification JSON path is empty");
  }
  return {
    inputPath,
...
async function main(): Promise<void> {
  ...
  const input = JSON.parse(source) as VerificationInput;
...
main().catch((error: unknown) => {
  ...
  console.error(`worldcut: ${message}`);
  process.exitCode = 1;
});
